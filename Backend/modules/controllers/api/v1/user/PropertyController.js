...
module.exports = new class PropertyController extends Controller {
    ...
    deleteProperty(req, res) {
     ...
     getAllPropertyByUser(req, res) {
        this.model.Property.find({userID:req.params.id}).sort({date:-1})
    ...
    this.model.Property.count({userID:req.params.id}).exec((err, Result) => {
     ...
     getAllProperty(req, res) {
        this.model.Property.find().sort({updatedAt:-1})
        .populate({path: 'PropertyType SubPropertyType TransactionType Agent User Discount'})
        .exec((err, Result) => {
            if (err) throw err;
            if (Result) {
    ...
    getProperty(req, res) {
        ...
        .populate({path: 'PropertyType SubPropertyType TransactionType Agent User Discount',populate:{path: 'SubPropertyType Pethos AgentLevel',populate:{path:'SubPethos',populate:{path:'SubSubPethos'}}}})
        .exec((err, Result) => {
            if (err) throw err;
            if (Result) {
                return res.json({
    ...
    advanceSearchProperty(req, res) {
        ...
        if (req.body.propertyTypeID) {
            query.propertyTypeID = req.body.propertyTypeID;
        }
        if (req.body.subPropertyTypeID) {
            query.subPropertyTypeID = req.body.subPropertyTypeID;
        }
        ...
        if (req.body.bath) {
         ...
         if (req.body.features) {
            query.features = {$elemMatch:{title:{$in:req.body.features}}};
        }
         if (req.body.condition) {
         query.condition={$elemMatch:{title:{$in:req.body.condition}}};
        }
         if (req.body.exchange) {
            query.exchange = {$elemMatch:{title:{$in:req.body.exchange}}};
        }
    ...
    newestProperty(req, res) {
        this.model.Property.find({}).sort({date:-1}).limit(10)
        .populate({path: 'PropertyType SubPropertyType TransactionType Agent User Discount'})
        .exec((err, Property) => {
            if (err)
                res.json({
   ...
   allPropertyBySearch(req, res) {
    ...
    allPropertyByPropertyTypeID(req, res) {
        this.model.Property.find({categoryID:req.params.id})
        .populate({path: 'PropertyType SubPropertyType TransactionType Agent User Discount'})
        .exec((err, Property) => {
            if (err)
                res.json({
    ...
    allPropertyBySubPropertyTypeID(req, res) {
        this.model.Property.find({subCategoryID:req.params.id})
        .populate({path: 'PropertyType SubPropertyType TransactionType Agent User Discount'})
        .exec((err, Property) => {
            if (err) throw err;
            if (Property) {
